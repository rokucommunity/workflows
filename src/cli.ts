...
let options = yargs
    .command('initialize-release', 'Initialize a release PR, draft GitHub release', (builder) => {
        return builder
            ...
            .option('preid', { type: 'string', description: 'Support for branching releases. Example value alpha or beta.', default: '' })
            .options('testRun', { type: 'boolean', description: 'Run the release in test mode' })
    }, (argv) => {
        if (!['major', 'minor', 'patch', 'pre', 'premajor', 'preminor', 'prepatch'].includes(argv.releaseType)) {
